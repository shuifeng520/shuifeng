...
def entry() -> None:
    ...
    c.rounded((1444, 842, 1876, 980), 22, COLORS["orange"], COLORS["orange_line"])
    c.circle(1482, 880, 18, "#fff8f0", COLORS["orange_line"])
    c.text((1482, 880), "建", 15, COLORS["orange_text"], "mm")
    c.text((1516, 872), "快捷建议", 22, COLORS["text"])
    c.text((1516, 906), "上传政策文件后，", 16, COLORS["orange_text"], max_width=300)
    c.text((1516, 930), "系统自动抽取规则，", 16, COLORS["orange_text"], max_width=300)
    c.text((1516, 954), "并生成审计关注点。", 16, COLORS["orange_text"], max_width=300)
    c.save()
