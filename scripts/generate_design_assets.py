...
def entry() -> None:
    ...
    c.rounded((1444, 808, 1876, 966), 22, COLORS["orange"], COLORS["orange_line"])
    c.circle(1482, 852, 18, "#fff8f0", COLORS["orange_line"])
    c.text((1482, 852), "建", 15, COLORS["orange_text"], "mm")
    c.text((1516, 844), "快捷建议", 22, COLORS["text"])
    c.text((1516, 878), "上传政策文件后，", 16, COLORS["orange_text"], max_width=300)
    c.text((1516, 902), "系统自动抽取规则，", 16, COLORS["orange_text"], max_width=300)
    c.text((1516, 926), "并生成审计关注点。", 16, COLORS["orange_text"], max_width=300)
    c.save()
